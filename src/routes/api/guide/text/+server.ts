  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        const retrievedGuides = await getGuides(categorizationResult.result.flatMap((object) => object.category))
        
        const organizedGuides = categorizationResult.result.flatMap((object): GuideResponse['data']['guide'] => {
